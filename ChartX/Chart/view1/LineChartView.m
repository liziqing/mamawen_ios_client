...
- (void)strokeChart
{
    if([_bzqjvalue count] == 0) {
        return;
    }
    UIBezierPath *path = [UIBezierPath bezierPath];
    UIBezierPath* fill = [UIBezierPath bezierPath];
    
    for(int i=1;i<_bzqjvalue.count;i++) {
     
        CGPoint p1 = CGPointMake((i-1)* (_axisWidth1),_axisHeight-[self.bzqjvalue[i-1]floatValue]);
        
        CGPoint p2 = CGPointMake(i *_axisWidth1, _axisHeight-[self.bzqjvalue[i]floatValue]);
