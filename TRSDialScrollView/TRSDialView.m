...
const CGFloat kTRSDialViewDefaultMinorTickDistance = 6.0f;
...
- (void)setDialRangeFrom:(NSInteger)from to:(NSInteger)to {
    ...
    frame.size.width = (_maximum - _minimum) * _minorTickDistance * 10  + self.superview.frame.size.width ;
...
- (void)drawTicksWithContext:(CGContextRef)context atX:(int)x
{
    ...
    if ([self isMajorTick:x]) {
        ...
        NSString *text = [NSString stringWithFormat:@"%.1f", value * 0.1];
